...
def log(log_type, msg=None, asset=None, new_asset=None, request=None):
    ...
    if log_type == "upline":
        ...
        event.name = "%s <%s> :    审批失败" % (new_asset.name, new_asset.sn)
...
class ApproveAsset:
    ...
    def _create_RAM(self, asset):
        ...
        for ram_dict in ram_list:
            if not ram_dict.get('slot') and "Virtual" not in asset.model:
...
class UpdateAsset:
    ...
    def _update_manufacturer(self):
        ...
        if m:
            manufacturer_obj = models.Manufacturer.objects.get_or_create(name=m)
    ...
    def _update_disk(self):
        ...
        if new_disks_dict:
            for key in new_disks_dict:
                ...
                if interface_type not in ['SATA', 'SAS', 'SCSI', 'SSD']:
                    interface_type = 'unknown'
                defaults = {
                    'slot': new_disks_dict.get('slot'),
                    'model': new_disks_dict.get('model'),
                    'manufacturer': new_disks_dict.get('manufacturer'),
                    'capacity': new_disks_dict.get('capacity'),
